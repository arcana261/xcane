...
class XCaneSynchronIterable {
   ...
   * @param {Mapper|string} fn - function to call on each object
   * @return {XCaneSynchronIterable} - reference to result
   */
  select(fn) {
    if (type.isString(fn)) {
      fn = v => objectPath.get(v, fn);
    }
  ...
  /**
   * @desc internal method to calculate minimum value in collection
   * @param {Comparer|string|Array.<string>} [fn] - comparer to use
   * @return {{first: boolean, current: *}} - result of reduction
   * @private
   */
  _min(fn) {
    fn = comparer.ascending(fn);

    return this.accumulate({first: true, current: null}, (prev, v, i) => {
      if (prev.first === true) {
        return {
          first: false,
          current: v
  ...
  /**
   * @desc returns minimum element found in collection or a default value
   * @param {Comparer|string|Array.<string>} fn - how to compare elements
   * @param {*=} value - default value to use if collection is empty
   * @return {*} - minimum value in collection or default value
   */
  minOrValue(fn, value) {
    const result = this._min(fn);
  ...
  /**
   * @desc returns minimum element found in collection or a null value
   * @param {Comparer|string|Array.<string>} [fn] - how to compare elements
   * @return {*|null} - minimum value in collection or a null
   */
  minOrNull(fn) {
    return this.minOrValue(fn, null);
  }

  /**
   * @desc returns minimum element found in collection or throws an exception
   * @param {Comparer|string|Array.<string>} [fn] - how to compare elements
  ...
  /**
   * @desc returns maximum value in collection or a default value
   * @param {Comparer|string|Array.<string>} fn - comparer to use
   * @param {*} value - default value to return if collection is empty
   * @return {*} - maximum value in collection or a default value
   */
  maxOrValue(fn, value) {
    return this.minOrValue(comparer.descending(fn), value);
  }

  /**
   * @desc returns maximum value in collection or a null value
   * @param {Comparer|string|Array.<string>} [fn] - comparer to use
   * @return {*|null} - maximum value in collection or a null value
   */
  maxOrNull(fn) {
    return this.maxOrValue(fn, null);
  }

  /**
   * @desc returns maximum value in collection or throws an error
   * @param {Comparer|string|Array.<string>} [fn] - comparer to use
   * @return {*} - maximum value in collection
   */
  max(fn) {
    return this.min(comparer.descending(fn));
  }

  /**
   * @desc skips items while a condition is set
   * @param {Predicate} fn - function to check
   * @return {XCaneSynchronIterable} - new collection
   */
  skipWhile(fn) {
    let hasReached = false;

    return this.where((v, i) => {
      if (hasReached) {
        return true;
      }
  ...
  /**
   * @desc skips items from start of collection until a condition comes true
   * @param {Predicate} fn - predicate to check
   * @return {XCaneSynchronIterable} - new collection
   */
  skipUntil(fn) {
    return this.skipWhile((v, i) => !fn(v, i));
  }

  /**
   * @desc skips numbers of elements from start of array
   * @param {number} n - number of elements
   * @return {XCaneSynchronIterable} - new collection
   */
  skip(n) {
    return this.skipWhile((v, i) => i < n);
  }

  /**
   * @desc concats to multiple iterable collections
   * @param {...ES6Iterable} args - arguments
   * @return {XCaneSynchronIterable} - new concatenated collection
   */
  concat(args) {
    return (new XCaneSynchronIterable([this].concat(Array.from(arguments))))
      .flatten();
  }

  /**
   * @desc permutes items
   * @return {XCaneSynchronIterable} - new permuted collection
   */
  permute() {
    ...
    return new XCaneSynchronIterable(arr);
  }
}
...
class XCaneAsynchronIterable extends Promise {
  ...
  /**
   * @desc flattens sub arrays
   * @return {XCaneAsynchronIterable} - new collection
   */
  flatten() {
    let mappedValue = [];
...
class XCaneIterable {
  ...
  /**
   * @desc generates a new range
   * @param {number} low - starting number
   * @param {number} high - ending number
   * @param {number} increment - step to take
   * @return {ES6Iterator} - new iterator
   */
  static range(low, high, increment) {
    if (!type.isNumber(increment)) {
      increment = 1;
    }

    /**
     * @desc generates a range of items
     */
    function * rangeGenerator() {
      for (let i = low; increment > 0 ? i < high : i > high; i += increment) {
        yield i;
      }
    }
